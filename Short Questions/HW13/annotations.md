...
## SECURITY
- **`@EnableWebSecurity`** - Enables Spring Security's web security support and customize security config for HTTP requests
- **`@EnableGlobalMethodSecurity`** - Enables method-level security annotations like `@PreAuthorize`, `@PostAuthorize`, `@Secured`, and `@RolesAllowed`
- **`@PreAuthorize` - Checks authorization before method execution using SpEL (expression)
- **`@PostAuthorize` - Checks authorization after method execution (can access return value).
- **`@Secured` - Checks for specific roles (no SpEL, just strings).