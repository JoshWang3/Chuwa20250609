...
Benefits:
- No password stored or managed by the app.
- One login = access to many apps (SSO).


Flow Summary:

1. User clicks “Sign in with Google”  
   → App redirects to: `https://accounts.google.com/o/oauth2/auth`
...
3. Google asks for consent   
   → User grants permission (e.g., profile, email).


4. Google redirects back with an auth code    
   → Code sent to app’s configured redirect URI.


5. App exchanges code for tokens   
   → Sends code + client ID + client secret to: `https://oauth2.googleapis.com/token`


6. Google responds with tokens  
    - `ID token` (JWT): user identity info
    - `Access token`: access Google APIs


7. App verifies ID token   
   → Parses JWT to extract user info (email, name).


8. User is now authenticated    
   → App creates local session or issues its own JWT.
...
---
#### 23. Compare `root certificate` and `certificate`.

---

|               | Root Certificate                          | Regular Certificate                      |
|---------------|--------------------------------------------|------------------------------------------|
| Issued By     | Self-signed (by the Certificate Authority) | Issued by a trusted CA or intermediate CA|
| Purpose       | Establish trust anchor in certificate chain| Authenticate identities (websites, apps) |
| Trust Level   | Most trusted (top of chain)                | Trusted if chain leads to trusted root   |
| Location      | Pre-installed in OS/browser trust store    | Delivered during SSL/TLS handshake       |
| Validates     | Intermediate and end-entity certificates   | Only the entity it is issued to          |
