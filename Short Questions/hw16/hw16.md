...
Modify `KafkaConsumerConfig.java`  
...
```
A message like "fail" would be lost if exception is thrown.



2. At Least Once (preferred)
- Kafka commits offset after successful processing.
- If failure occurs before commit → message is reprocessed (possible duplication).
...
- Requires Kafka transaction support on producer
- Consumer side needs idempotency and/or transactional guarantees
...
---
I will write my own partitioning logic:  
``` 
If no key is provided, all messages will go to partition 0 by default.
```
...
public class CustomPartitioner implements Partitioner {
    ...
    public int partition(String topic, Object keyObj, byte[] keyBytes,
                         Object valueObj, byte[] valueBytes, Cluster cluster) {
...
Update `application.properties` to use the custom partitioner:
```properties
spring.kafka.producer.properties.partitioner.class=com.chuwa.demo.kafka.CustomPartitioner
```

Outcome:
![custom_partitioner_postman](images/F_custom_partitioner_postman.png)


![custom_partitioner_ui](images/F_custom_partitioner_ui.png)
