...
# output should be: -rw-r--r--@ 1 gloriaupup  staff  2276 Jul 15 23:36 src/main/resources/springhttps-keystore.jks
...
```bash
curl -u admin:password https://localhost:8443/api/hello
...
- Reason: Self-signed cert not in system trust store

#### This is because :

  1. Certificate Chain Validation
     - TLS clients verify certificates against trusted Certificate Authorities (CAs)
     - System trust store contains certificates from known CAs (VeriSign, Let's
       Encrypt, etc.)
  2. Self-Signed Certificate Problem
     - Owner = Issuer (CN=localhost signed by CN=localhost)
     - No CA Signature - Certificate signed by itself, not a trusted CA
     - Chain of Trust Broken - No path to a trusted root CA
  3. Client Behavior
     - curl checks system trust store (/etc/ssl/certs/, Keychain on macOS)
     - My springhttps-keystore.jks is NOT in system trust store
     - Security protocols reject untrusted certificates by default
#### Normal Certificate Chain:
```text
Website Certificate → Intermediate CA → Root CA (in system trust store)
```
#### Self-Signed Certificate:
```text
springhttps certificate → springhttps certificate (self-signed)
                        ↓
            NOT in system trust store ❌
```
Result: ❌ Certificate verification fails because there's no trusted path to
validate the certificate's authenticity.


### 3. Make HTTPS Work WITHOUT Bypassing TLS/SSL

#### Custom TrustManager Approach (HttpsClient.java): ✅
```java
 // Load the keystore
KeyStore trustStore = KeyStore.getInstance("JKS");
        trustStore.load(new FileInputStream("src/main/resources/springhttps-keystore.jks"), 
                       "password".toCharArray());

// Create TrustManager that uses my keystore
...
#### System Trust Store Approach ❌

```java
// Use default HTTP client (uses system trust store)
...
try {
    ...
    System.out.println("This fails because the self-signed certificate is not in the system trust store.");
}
```

#### Test Results:
- ✅ Custom TrustManager: 200 OK with proper certificate validation
- ❌ System Trust Store: certificate_unknown (expected failure)
  
![screenshots](screenshots/TrustManager.png)

Why This is NOT Bypassing TLS/SSL

- Full Certificate Validation: Cryptographic signature verification maintained
- Hostname Verification: CN=localhost matches connection hostname
- TLS Encryption: All traffic encrypted with TLS 1.3
- Trust Store Extension: Adding cert to custom trust store, not disabling validation
